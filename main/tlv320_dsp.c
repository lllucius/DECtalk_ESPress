...
esp_err_t tlv320_dsp_apply(const tlv320_dsp_state_t *state)
{
    ...
    // Mute briefly so coefficient writes can't be heard as clicks.
    // On failure we leave the codec muted and let the caller decide
    // how to recover (usually by re-running apply with the last
    // known-good state).
